...
public class FastScroller extends LinearLayout {
    ...
    private void setRecyclerViewPosition(float y)
    {
        if(recyclerView!=null)
        {
            ...
            int targetPos = Math.round(y*itemCount/height);
            if(targetPos < 0)
                targetPos = 0;
            else if(targetPos > itemCount - 1)
    ...
    private class ScrollListener extends RecyclerView.OnScrollListener
    {
        ...
        public void onScrolled(RecyclerView rv,int dx,int dy)
        {
            if(!ScrollInProgress) {
                ...
                Log.d("position", String.valueOf(firstVisiblePosition));
                ...
                visibleRange = (int) Math.ceil((float) height / px);

                float proportion = ((float) lastVisiblePosition - (float) visibleRange - 1) / ((float) itemCount - (float) visibleRange);
